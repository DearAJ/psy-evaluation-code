  ...
  <div class="warning-database-container" 
    ...
    <div class="content-container">
      <el-table :data="warningData" border
        ...
        <el-table-column align="center" label="档案" width="65px">
          <template slot-scope="scope">
            <el-button size="mini" @click="psyStudentFiles(scope.row)">查看</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    psyStudentFiles(row) {
      console.log(row)
      this.$router.push({name: 'FileDetails', query: {studentId:row.stuId,identity:'student',name:row.stuName,classes:row.classes,grade:row.grade,userId:row.userId,school:row.schoolName}})
    },
    handleChange(row) {
      if(row.selected) {
    ...
    updateDetails(row) {
      ...
      if(this.selectedStudents.length == 0) {
        ...
        const headers = ['学段', '年级', '班级', '任务名称', '姓名', '更新时间', '下次干预截止时间', '心理老师', '当前状态', '是否转介', '去向/处理', '所属学期'];
        const val = ['stuPeriod', 'stuGrade', 'stuClass', 'taskName', 'stuName', 'createTime', 'expireTime', 'psyTeacher', 'stuState', 'isTransfer', 'postProcessing', 'semester'];
        const title = '预警初筛库学生导出数据';
        let exportData = [];
          if(this.isSelectedAll) {
            try {
